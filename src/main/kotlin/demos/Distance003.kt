    ...
    var frames = 0
    {
        drawer.isolated {
            ...
            val directions = jumpFlooder.result.shadow.flatMapIndexed(
                    (0 until jumpFlooder.width step 1),
                    (0 until jumpFlooder.height step 1)
            ) { x, y, r, g, _, _ ->

                val dist = Vector2(r, g)
                val direction = (rotateZ(x  * 4.0 + seconds * 90.0) * dist.xy01).xyz.xy.normalized * 4.0
