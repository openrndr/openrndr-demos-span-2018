        ...
        val contours = shapeNodes.flatMap {
            ...
            it.shape.contours.flatMap {
                listOf(0,3).map { k ->
                    ...
                    val offset = it.offset(k * 10.0, SegmentJoin.ROUND).sub(0.0, Math.cos(shapeIndex+seconds+k*Math.PI/10.0)*0.5+0.5)
                    offset.transform(transform {
                        translate(0.0, Math.sin(shapeIndex+seconds)*140.0)

                        translate(c)
                        rotate(Vector3.Companion.UNIT_Z, Math.cos(shapeIndex+seconds)*15.0)
                        translate(-c)
                    })
                }
            }
